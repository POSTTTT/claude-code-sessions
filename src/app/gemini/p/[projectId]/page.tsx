...
import { listGeminiSessions, resolveGeminiProjectPath } from "@/lib/gemini";
...
export default async function GeminiProjectPage({
  params,
}: {
  ...
  const [sessions, realPath] = await Promise.all([
    listGeminiSessions(decoded),
    resolveGeminiProjectPath(decoded),
  ]);
  return (
    <div>
      <Link href="/gemini" className="text-sm text-white/60 hover:text-white">
        ← All Gemini projects
      </Link>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-white/50">
                  ...
                  <span>
                    {formatNumber(s.inputTokens)} ctx /{" "}
              ...
              <DeleteButton
                target={`gemini-session:${decoded}:${s.sessionId}`}
                label="Delete"
                confirm="Permanently delete this Gemini session? This cannot be undone."
              />
            </div>
          </div>
        ))}
